...
namespace esphome {
    namespace bus_t4 {
        ...
        HardwareSerial CanSerial(2); // Utilisation de UART1 sur ESP32
        ...
        void NiceBusT4::send_array_cmd(const uint8_t *data, size_t len) {
            ...
            Serial.flush();  // Effacer le tampon de l'UART
            Serial.updateBaudRate(BAUD_BREAK);  // Réduire le débit de l'UART
            Serial.write((const uint8_t *)&br_ch, 1);  // Envoyer un zéro à une vitesse plus lente, un zéro long
            ...
            // send payload itself
            Serial.updateBaudRate(BAUD_WORK);  // Rétablir le débit de l'UART
            Serial.write(data, len);  // Envoyer le paquet principal
            while (Serial.availableForWrite() < len); // Attendre la fin de l'envoi
